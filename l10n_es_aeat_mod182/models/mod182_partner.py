...
# Copyright 2025 Fundesplai - Pol Canelles <pol.canelles@fundesplai.org>
            ...
            if (
                ...
                raise exceptions.ValidationError(
                    _(
                        ...
                        "required because "
                        "'CLAVE' is set as %(key)s and "
                        "'REVOCACIÓN' is set as %(revocation)s."
                    )
                    % {"key": rec.operation_key, "revocation": rec.revocation}
                )
            ...
            if (
                ...
                raise exceptions.ValidationError(
                    _(
                        ...
                        "'NATURALEZA DEL DECLARANTE' is set as %(nature)s "
                        "and 'CLAVE' is set as %(key)s."
                        % {
                            "nature": rec.report_id.declarant_nature,
                            "key": rec.operation_key,
                        },
                    )
                )
            elif (
